...
def donations():
    ...
    if request.method == "GET":
        ...
        if request.args.get('n'):
            ...
            query = (" SELECT * FROM " +
                    " (SELECT amount, convert_tz(cretime, '+0:00', '-07:00') as date, status, type, donorEmail, formName, donorName FROM funraise_donations_friends WHERE formName <> 'Facebook' " +
                    " UNION ALL SELECT amount, convert_tz(cretime, '+0:00', '-07:00') as date, status, type, donorEmail, formName, donorName FROM funraise_donations_arc WHERE formName <> 'Facebook' " +
                    " UNION ALL SELECT amount, convert_tz(cretime, '+0:00', '-07:00') as date, IF(amount > 0, 'Complete', 'Refunded') as status, 'One Time' as type, donorEmail, concat('Facebook - ', page) as formName, name as donorName FROM `facebook_donations`) temp ")
        ...
        rows = db_query(("SELECT amount, convert_tz(cretime, '+0:00', '-07:00') as date, status, type, donorEmail, formName, donorName FROM funraise_donations_friends WHERE donorName not like '%for good%' and formName <> 'Facebook' " +
                        " UNION ALL SELECT amount, convert_tz(cretime, '+0:00', '-07:00') as date, status, type, donorEmail, formName, donorName FROM funraise_donations_arc WHERE donorName not like '%for good%' and formName <> 'Facebook' " +
                        " UNION ALL SELECT amount, convert_tz(cretime, '+0:00', '-07:00') as date, IF(amount > 0, 'Complete', 'Refunded') as status, 'One Time' as type, donorEmail, concat('Facebook - ', page) as formName, name as donorName FROM `facebook_donations` " +
